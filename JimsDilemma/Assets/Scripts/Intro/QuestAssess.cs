...
public class QuestAssess : MonoBehaviour,IPointerEnterHandler,IPointerExitHandler {
    ...
    public static QuestAssess Instance //=> instance ? instance : this;
    { get  { return instance; } }

    private static QuestAssess instance;
    ...
    //[SerializeField] private TaskList masterTaskList;
    ...
    void Awake(){
        ...
        if (instance)
        {
            Debug.Log("There are two instances of QuestAssess");
        ...
        Transform tempParent = transform.GetChild(0);//.parent.parent.parent;
		//FIXME This may be causing errors to appear on console NullReferenceExemption(Null);
		if(navigationButtons.Length == 0)
		navigationButtons = tempParent.GetComponentsInChildren <Button>(true);

		if (questScrollBar == null)
		questScrollBar = tempParent.parent.GetComponentInChildren<Scrollbar> (true);
	...
	void OnEnable(){
        ...
        //foreach (var task in DATA_MANAGER.masterTaskList.Items)
        //{
...
//	}
	...
	public void OnUpdate(){
        ...
        //
        //		}	
        foreach (var t in DATA_MANAGER.playerData.masterTaskList.Items)
        {
       
            var curTaskStatus = t.taskStatus;

            if (curTaskStatus == Task_Status.NOT_IDENTIFIED)
                continue;
            //foreach(KeyValuePair<Task,Task_Status> t in DATA_MANAGER.playerData.taskDictionary){

            if (Task_Status.IDENTIFIED == curTaskStatus){

				qTextSpaces [count].text = t.taskDescription;
				qTextSpaces [count].color = Color.gray;


			}

			if(Task_Status.COMPLETED == curTaskStatus){
