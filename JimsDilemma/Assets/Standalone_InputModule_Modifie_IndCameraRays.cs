...
namespace UnityEngine.EventSystems
{
    ...
    public class Standalone_InputModule_Modifie_IndCameraRays : PointerInputModule
    {
        ...
        [SerializeField]
        private Transform positionToStartLine;

        private bool m_isClickTrigger;
        ...
        public IEnumerator RendLateUpdate(Vector3 pos)
        {
            lineRenderer.SetPosition(0, positionToStartLine.position);
            
            //Limits on how short the line can be 
            if (1.5f * 1.5f < pos.sqrMagnitude - positionToStartLine.position.sqrMagnitude)
                lineRenderer.SetPosition(1, pos);
            else //SET DEFAULT LINE RENDER
                lineRenderer.SetPosition(1, positionToStartLine.position + positionToStartLine.forward * 10f);
        ...
        public void SetInputState(bool inputState){
        ...
        public override void Process()
        {
            ...
            //FIXME  1/11/20 - Removed from original. Allows for main camera to receive update without clicking update.
            //if (!eventSystem.isFocused && ShouldIgnoreEventsOnNoFocus())
            //    return;
            //StartCoroutine(RendLateUpdate(positionToStartLine.position + (positionToStartLine.forward * 10)));
        ...
        protected void ProcessMousePress(MouseButtonEventData data)
        {
            ...
            if (data.PressedThisFrame())
            {
                ...
                //FIX ME ADDED LINE RENDERER

                //   lineRenderer.SetPosition(0, positionToStartLine.position);
                //if (2 < pointerEvent.pointerCurrentRaycast.worldPosition.magnitude)
                //{
                // if(pointerEvent.pointerCurrentRaycast.distance > 0.2f)
                StartCoroutine(RendLateUpdate(positionToStartLine.position + (positionToStartLine.forward * pointerEvent.pointerCurrentRaycast.distance)));
                 //   lineRenderer.SetPosition(1, positionToStartLine.position + (positionToStartLine.forward * pointerEvent.pointerCurrentRaycast.distance));
                  //  cachePositionLR = pointerEvent.pointerCurrentRaycast.worldPosition;
             //   }
